...
class Filter:
    ...
    def command(
        ...
        async def filter_func(client, event) -> bool:
            ...
            # Intercept disabled commands
            chat_id = getattr(event, "chat_id", getattr(getattr(event, "message", None), "chat_id", None))
            if chat_id and chat_id < 0:
                from ._database import db  # Use relative import to avoid circular dependencies
                from ._group_db import group_db
                is_disabled = await group_db.is_command_disabled(chat_id, cmd.lower())
                if is_disabled:
                    return False
