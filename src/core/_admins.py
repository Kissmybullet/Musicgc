...
async def load_admin_cache(
    ...
    admin_list = await c.searchChatMembers(
        chat_id=chat_id, filter=types.ChatMembersFilterAdministrators()
    )
...
async def check_permissions(
    c: Client, chat_id: int, user_id: int, permissions: PermissionsType
) -> bool:
    ...
    if await is_owner(c, chat_id, user_id):
        return True
    ...
    _, user_info = await get_admin_cache_user(c, chat_id, user_id)
    if not user_info:
        return False
...
async def is_owner(c: Client, chat_id: int, user_id: int) -> bool:
    ...
    """
    is_cached, user = await get_admin_cache_user(c, chat_id, user_id)
    if not user:
        return False
...
async def verify_anonymous_admin(
    ...
    if not await check_permissions(
        c, message.chat_id, callback.sender_user_id, permissions
    ):
    ...
    await c.deleteMessages(chat_id=message.chat_id, message_ids=[callback.message_id])
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        async def wrapper(
            ...
            **kwargs,
            ...
            if only_owner and not await is_owner(c, chat_id, user_id):
                if no_reply:
                    return None
            ...
            async def check_and_notify(
                ...
                if not await is_admin(c, chat_id, subject_id):
                    if no_reply:
                        return None
                    await sender(f"{subject_name} needs to be an admin.")
                    return False

                if not await check_permissions(c, chat_id, subject_id, permissions):
                    if no_reply:
                        return None
                    await sender(
            ...
            if is_auth:
                ...
                is_admin_user = await is_admin(c, chat_id, user_id)
