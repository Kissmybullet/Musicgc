...
class Calls:
            ...
            if await db.get_logger_status(self.bot.me.id):
                self.bot.loop.create_task(
                    send_logger(
                        client=self.bot,
                        chat_id=chat_id,
                        song=chat_cache.get_playing_track(chat_id),
                    )
                )
    ...
    async def _play_song(self, chat_id: int, song: CachedTrack) -> None:
        ...
        try:
            ...
            # Parse text entities
            parse = await self.bot.parseTextEntities(
                text=text, parse_mode=types.TextParseModeHTML()
            )
            ...
            if thumbnail:
                ...
                await self.bot.editTextMessage(
                    chat_id=chat_id,
                    message_id=reply.id,
                    text=text,
                    parse_mode="html",
    ...
    async def _join_ub(self, chat_id: int) -> Union[types.Ok, types.Error]:
        ...
        if not invite_link:
            get_link = await self.bot.createChatInviteLink(
                chat_id=chat_id, name="MelodyForgeBot"
            )
        ...
        invite_link_str = (
            ...
            else str(invite_link)
        )
        invite_link_str = invite_link_str.replace(
            "https://t.me/+", "https://t.me/joinchat/"
        )
        user_id = client.me.id
        cache_key = f"{chat_id}:{user_id}"
        try:
            await client.join_chat(invite_link_str)
