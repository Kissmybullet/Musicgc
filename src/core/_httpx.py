...
class HttpxClient:
    ...
    def __init__(
        ...
        if HttpxClient._shared_session is None or HttpxClient._shared_session.closed:
            HttpxClient._shared_session = aiohttp.ClientSession(
                timeout=self._request_timeout,
                connector=aiohttp.TCPConnector(),
            )
        self._session = HttpxClient._shared_session

    async def close(self) -> None:
        """Closes the underlying aiohttp session gracefully."""
        try:
            if HttpxClient._shared_session and not HttpxClient._shared_session.closed:
                await HttpxClient._shared_session.close()
                HttpxClient._shared_session = None
    ...
    async def download_file(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            return DownloadResult(success=False, error=error_msg, status_code=e.status)
